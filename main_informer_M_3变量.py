...
def get_true_data(sheet_name,true_file,arg):
    # data = pd.read_excel('./TrueValue/most_samll_test_3个变量真实值.xlsx', encoding='UTF-16')
...
def initialize_parameter():
    ...
    parser.add_argument('--target', type=str, default='X3', help='S或MS任务中的目标特征列名（target feature in S or MS task）')
    ...
    parser.add_argument('--inverse', action='store_true', help='逆标准化输出数据 inverse output data', default=False)
    ...
    parser.add_argument('--pred_len', type=int, default=48, help='预测序列长度（prediction sequence length）原始默认为24------------------预测序列长度pred_len')
    ...
    parser.add_argument('--sub_them', type=str, default='3变量多对多', help='单次运行的存储文件夹字后面的内容--------------------存储数据父文件夹名字')
    ...
    parser.add_argument('--true_file', type=str, required=False, default='./TrueValue/most_samll_test_3个变量真实值.xlsx', help='真实值数据的文件名')

    # parser.add_argument('--data_path', type=str, default='周粒度-多特征数据汇总.csv', help='data file')
    parser.add_argument('--data_path', type=str, default='most_samll_test_3个变量.csv', help='data file')

    parser.add_argument('--columns', type=list, required=False, default=["date",'X1','X2','X3'], help='存储预测数据的时候的列名，多对多M')
...
if __name__ == '__main__':
    ...
    data_parser = {
        ...
        'chicken_S': {'data': '月均价.csv', 'T': 'price', 'M': [2, 2, 2], 'S': [1, 1, 1], 'MS': [2, 2, 1]},
